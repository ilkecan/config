  ...
  mkFlakePartsScope = self':
    {
      ...
      options-list-file = mkOptionsList { inherit (self'.debug) options; excluded = [ "perSystem" ]; };
    ...
    optnix = {
      ...
      settings = {
        ...
        scopes = {
          ...
          (genAttrs' self'.debug.systems (x: nameValuePair "flake-parts.${x}" (mkFlakePartsPerSystemScope self' x)))
